  ...
  const handleToggleRewardStatus = async (
    ...
    try {
      if (
        ...
        showToast.error("No se puede activar una recompensa sin stock");
        return;
      }
      if (
        reward.expirationDate !== null &&
        reward.expirationDate !== undefined &&
        new Date(reward.expirationDate) < new Date()
      ) {
        showToast.error(
          "No se puede activar una recompensa con fecha de expiración pasada"
        );
        return;
      }
      const response = await api.rewards.update(reward.id, {
        active: newStatus,
      });
      ...
      if (response.success) {
        showToast.success(
          response.message ||
            `Recompensa ${newStatus ? "activada" : "desactivada"} exitosamente`
        );
        loadData();
      } else {
        showToast.error(
          response.error ||
            `Error al ${newStatus ? "activar" : "desactivar"} la recompensa`
        );
      }
    } catch (error) {
      console.error("Error:", error);
                        ...
                        <div className="flex items-center space-x-2">
                          <Switch
                            ...
                            onCheckedChange={(checked) =>
                              handleToggleRewardStatus(reward, checked)
                            }
